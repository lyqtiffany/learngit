...
#我们做UI自动化，经常会出现NoSuchElementException,也就是找不到元素异常
#这个报错的原因有4个
  #1，表达式确实写错了，或者元素确实不存在
      # a. 表达式所描绘的元素确实是不存在的
      # b. 循环元素列表的时候，列表里的每一个都找了一遍，但是并不是列表里的每个元素都有。
        #比如，列表里面有10个p标签，但是只有前5个拥有子元素a标签，所以在循环到第六个的时候，就会报错，找不到a标签
  #2 元素未加载，用显示等待或者隐式等待，（注意xpath效率极慢，所需时间更久）
  #3内嵌网页
  #4多标签页面

  #关于什么时候用什么等待
     #元素未加载而导致的问题，用显示等待或者隐式等待
     #浏览器或者电脑，性能不高，反应不过来，用强制等待
